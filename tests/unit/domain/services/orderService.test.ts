...
describe('OrderService', () => {
  ...
  describe('getMyOrder', () => {
    ...
    it('debería fallar si orden no existe', async () => {
      ...
      await expect(OrderService.getMyOrder('user123', 'ORD-999')).rejects.toThrow(
        'Orden no encontrada'
      );
    });
  });
  ...
  describe('cancelOrder', () => {
    it('debería cancelar orden como customer exitosamente', async () => {
      ...
      (OrderModel.findOneAndUpdate as jest.Mock).mockResolvedValue(mockUpdatedOrder);
  ...
  describe('advanceOrderStatus', () => {
    ...
    it('debería fallar si orden no existe', async () => {
      ...
      await expect(OrderService.advanceOrderStatus('ORD-999', 'PROCESANDO')).rejects.toThrow(
        'ORDER_NOT_FOUND'
      );
    });
    ...
    it('debería fallar con transición inválida', async () => {
      ...
      await expect(OrderService.advanceOrderStatus('ORD-123', 'PROCESANDO')).rejects.toThrow(
        'INVALID_TRANSITION'
      );
    });
  ...
  describe('updateOrderTrackingStatus', () => {
    it('debería actualizar tracking status exitosamente', async () => {
      ...
      expect(result).toBeDefined();
      expect(result.trackingStatus).toBe(OrderStatus.ENVIANDO_A_TRANSPORTADORA);
    });
    ...
    it('debería fallar si orden no existe', async () => {
      ...
      await expect(
        OrderService.updateOrderTrackingStatus(
          'ORD-999',
  ...
  describe('customerConfirmDelivery', () => {
    ...
    it('debería fallar si orden no existe', async () => {
      ...
      await expect(OrderService.customerConfirmDelivery('user123', 'ORD-999')).rejects.toThrow(
        'ORDER_NOT_FOUND'
      );
    });
