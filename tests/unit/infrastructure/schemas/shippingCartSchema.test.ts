...
import { CartSchema } from '../../../../src/infrastructure/schemas/shippingCartSchema';
...
describe('ShippingCartSchema', () => {
  ...
  describe('CartSchema', () => {
    ...
    it('debería recalcular totales al validar', async () => {
      const cart = new CartModel({
        userId: 'user123',
        items: [
          {
    ...
    it('debería transformar fechas a ISO en toJSON', () => {
      const now = new Date();
      const cart = new CartModel({
        userId: 'user123',
        createdAt: now,
    ...
    it('debería transformar fechas a ISO en toObject', () => {
      const now = new Date();
      const cart = new CartModel({
        userId: 'user123',
        createdAt: now,
      ...
      expect(obj.createdAt).toMatch(/^\d{4}-\d{2}-\d{2}T/);
      expect(obj.updatedAt).toMatch(/^\d{4}-\d{2}-\d{2}T/);
    });

    it('debería tener índice único para userId y status active', () => {
      const indexes = CartSchema.indexes();
    ...
    it('debería permitir múltiples carritos closed para el mismo usuario mediante partial index', () => {
      const indexes = CartSchema.indexes();
      const uniqueActiveIndex = indexes.find(
        ([fields, options]) =>
          fields.userId === 1 &&
      ...
      expect(uniqueActiveIndex).toBeDefined();
      expect(uniqueActiveIndex?.[1]?.partialFilterExpression).toEqual({ status: 'active' });
    });
  });
});